...
export const useFileSystemStore = create<FileSystemState>((set, get) => ({
  ...
  moveItem: (itemId, targetId) =>
    set(
      produce((state) => {
        ...
        if (
          ("parent" in item && item.parent === null) ||
          ("folder" in item && item.folder === null)
        ) {
          state.rootItems = state.rootItems.filter(
            (id: string) => id !== itemId
          );
        }
  ...
  setFolderReadOnly: (folderId, isReadOnly) =>
    set(
      produce((state) => {
  ...
  getPath: (itemId) => {
    ...
    while (parentId) {
      ...
      path.unshift(parent);
      parentId =
        parent.cardType === "folder" ? (parent as FolderItem).parent : null;
    }
  ...
  getFolderById: (folderId) => {
    ...
    return folder && folder.cardType === "folder"
